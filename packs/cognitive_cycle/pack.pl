...
% State the fact: version('0.3.0') — slice 37 seats the night watchman: the two-process governor throws the switch in the live tick.
version('0.3.0').
...
% State the fact: requires the component packs it orchestrates (their dependencies come transitively), the bus it announces on, and since slice 37 the two-process governor it seats in the tick.
requires([drive_system, connection_graph, action_selector, override_controller, plasticity_engine, observer, neuromodulator_bus, two_process_governor]).
