...
% State the fact: version('0.4.0') — slice 38 seats the night crew: the tick reads the thrown switch and runs the offline works.
version('0.4.0').
...
% State the fact: requires the component packs it orchestrates (their dependencies come transitively), the bus it announces on, and since slice 37 the two-process governor it seats in the tick, and since slice 38 the offline consolidation engine the thrown switch commands.
requires([drive_system, connection_graph, action_selector, override_controller, plasticity_engine, observer, neuromodulator_bus, two_process_governor, offline_consolidation]).
